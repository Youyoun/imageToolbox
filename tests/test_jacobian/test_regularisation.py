...
class TestJacobianOperator:
    ...
    def test_batch_jacobian():
        ...
        assert torch.isclose(
            jacobs.squeeze().transpose(0, 1), batched_jacob.squeeze(), rtol=FLOAT_TOL
        ).all()
...
class TestEVComputation:
    ...
    def test_get_lambda_max_min(alpha: float):
        ...
        ev_min = get_lambda_min_or_max_poweriter(
            f, x, alpha, is_eval=True, biggest=False, n_iter=_niter
        )
        ev_largest_abs = get_lambda_min_or_max_poweriter(
            f, x, None, is_eval=True, biggest=True, n_iter=_niter
        )
...
class TestPenalization:
    ...
    def test_penalization_fulljacobian(eps: float):
        ...
        pen, ev_min = MonotonyRegularization(
            eps=eps,
            eval_mode=True,
    ...
    def test_penalization_powermethod_onevector(alpha: float, eps: float):
        ...
        f = lambda x: torch.matmul(W, x)
        pen, ev_min = MonotonyRegularization(
            method=PenalizationMethods.POWER,
            alpha=alpha,
            eps=eps,
            eval_mode=True,
            max_iter=_niter,
    ...
    def test_penalization_powermethod_batchvector(alpha: float, eps: float):
        ...
        pen, ev_min = MonotonyRegularization(
            method=PenalizationMethods.POWER, alpha=alpha, eps=eps, eval_mode=True, max_iter=_niter
        )(f, x)
...
class TestOptimizedPenalizationPowerMethodReLU:
    ...
    def test_penalization_optpowermethod_random_diag(alpha: float, eps: float):
        ...
        pen, ev_min = MonotonyRegularization(
            method=PenalizationMethods.OPTPOWER,
            alpha=alpha,
            eps=eps,
            eval_mode=True,
            max_iter=_niter,
    ...
    def test_penalization_powermethod_optpowermethod(alpha: float, eps: float):
        ...
        pen_opt, ev_min_opt = MonotonyRegularization(
            method=PenalizationMethods.OPTPOWER,
            alpha=alpha,
            eps=eps,
            eval_mode=True,
    ...
    def test_penalization_powermethod_optpowermethod_grads_percentclose(alpha: float, eps: float):
        ...
        grad_pm_opt = W.grad.clone()
        assert (
            torch.isclose(grad_pm, grad_pm_opt, rtol=0.0, atol=FLOAT_TOL).sum()
            > grad_pm.nelement() * PERCENT_CLOSE
        ), (
            f"||gradP - gradRC|| // ||gradP|| = {torch.norm(grad_pm_opt - grad_pm) / torch.norm(grad_pm)}, "
    ...
    @staticmethod
    @pytest.mark.parametrize(["alpha", "eps"], itertools.product(ALPHAS, EPS))
    def test_penalization_powermethod_optpowermethod_grads_percentclose_relu(
...
class TestOptimizedNoAlphaPenalizationPowerMethodReLU:
    ...
    def test_penalization_random_diag(alpha: float, eps: float):
        ...
        pen, ev_min = MonotonyRegularization(
            method=PenalizationMethods.OPTPOWERNOALPHA,
            alpha=alpha,
            eps=eps,
            eval_mode=True,
            max_iter=_niter,
    ...
    @staticmethod
    @pytest.mark.parametrize(["alpha", "eps"], itertools.product(ALPHAS, EPS))
    def test_penalization_powermethod_optpowermethod_noalpha_grads_percentclose(
        alpha: float, eps: float
    ):
        _niter = 1000
...
class TestConstraintValuesWithTransformedConstraint:
    ...
    def test_values_equality_relu(alpha: float, eps: float, method: PenalizationMethods):
        ...
        method_ = MonotonyRegularizationShift(
            method=method,
            alpha=alpha,
            eps=eps,
            eval_mode=True,
    ...
    def test_values_equality(alpha: float, eps: float, method: PenalizationMethods):
        ...
        f2 = lambda x: 2 * torch.matmul(W, x) - x
        method_ = MonotonyRegularizationShift(
            method=method, alpha=alpha, eps=eps, eval_mode=True, max_iter=_niter
        )
        method_2 = MonotonyRegularization(
            method=method, alpha=alpha, eps=-1 + eps, eval_mode=True, max_iter=_niter
        )
        pen1, ev_min1 = method_2(f2, x)
        pen2, ev_min2 = method_(f, x)
        ...
        assert torch.isclose(real_evs1[0], (ev_min2 + 1) / 2, atol=FLOAT_TOL, rtol=0.0).all()
        assert torch.isclose(
            torch.maximum(torch.ones(1) - eps, -real_evs2[0]), pen1, atol=FLOAT_TOL, rtol=0.0
        ).all()
...
class TestMonotonyLearningWithTransformedConstraint:
    ...
    def test_learning_monotony_power(alpha: float, eps: float, method: PenalizationMethods):
        ...
        model = nn.Linear(NDIM, NDIM, bias=True).to(
