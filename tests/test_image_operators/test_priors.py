...
class TestPriors:
    ...
    def test_total_variation_specific_vector(eps):
        x = torch.Tensor([[[2, 6, 2, 1],
                           ...
                           [5, 2, 0, 1],
                           [2, 5, 6, 6]]])
        grad_y = torch.Tensor([[[4, -4, -1, 0],
                                [3, -6, 8, 0],
                                [-3, -2, 1, 0],
                                [3, 1, 0, 0]]])
        grad_x = torch.Tensor([[[2, 1, -1, 8],
                                [1, -5, -1, -8],
                                [-3, 3, 6, 5],
                                [0, 0, 0, 0]]])
        tv_grad = torch.Tensor([[[-1.3413, 1.6216, 0.4408, -1.7053],
                                 [-0.8172, 2.5990, -2.3417, 2.9920],
                                 [1.7299, -1.6243, -1.8291, -1.8353],
                                 [-1.7064, 0.8361, 1.9813, 0.9998]]])  # Computed with EPS=1e-2
