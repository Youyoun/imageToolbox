...
class TestTransforms:
    ...
    def load_image_and_transform(transform):
        cat_im = Image.open(
            Path(__file__).parent.parent / "test_image_operators/chelseaColor.png"
        ).convert("L")
    ...
    def test_compose():
        transform = Compose(
            [Random90Rotation(p=1.0), RandomHorizontalFlip(p=1.0), RandomVerticalFlip(p=1.0)]
        )
        cat_t, flipped_cat_t, flipped_cat_t_2 = TestTransforms.load_image_and_transform(transform)
        assert are_equal(flipped_cat_t, flipped_cat_t_2)
        assert are_equal(
            flipped_cat_t,
            ttransforms.RandomVerticalFlip(p=1)(
    ...
    def test_compose_random_p(p: float):
        transform = Compose(
            [Random90Rotation(p=p), RandomHorizontalFlip(p=p), RandomVerticalFlip(p=p)]
        )
        cat_t, flipped_cat_t, flipped_cat_t_2 = TestTransforms.load_image_and_transform(transform)
        assert are_equal(flipped_cat_t, flipped_cat_t_2)
    ...
    def test_get_transform_with_object(p: float):
        transform = get_transforms(
            [
                (AvailableTransforms.Random90Rotation, {"p": p}),
                (AvailableTransforms.RandomVerticalFlip, {"p": p}),
            ]
        )
        assert (
            isinstance(transform.transforms[0], Random90Rotation)
            and transform.transforms[0].p == p
        )
        assert (
            isinstance(transform.transforms[1], RandomVerticalFlip)
            and transform.transforms[1].flip_prob == p
        )
