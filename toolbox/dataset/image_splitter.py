...
class ImageSplitter(Splitters):
    ...
    def split_tensor(self, tensor_: torch.Tensor) -> Tuple[torch.Tensor, Tuple[int, int], List[torch.Size]]:
        ...
        for x in range(0, tensor_.shape[1], patch_x_size):
            for y in range(0, tensor_.shape[2], patch_y_size):
                ...
                patches.append(
                    torch.nn.functional.pad(p, (0, patch_y_size - p.shape[2], 0, patch_x_size - p.shape[1])).unsqueeze(
                        0))
    ...
    def group_tensor(self, tensors: torch.Tensor, sizes: List[torch.Size]) -> torch.Tensor:
        ...
        for x in range(0, x_size, patch_x_size):
            for y in range(0, y_size, patch_y_size):
                final[..., x:x + patch_x_size, y:y + patch_y_size] = tensors[i, :sizes[i][0], :sizes[i][1],
                                                                     :sizes[i][2]]
                i += 1
        return final
...
class ImageSplitterOverlapIgnored(Splitters):
    ...
    def split_tensor(self, tensor_: torch.Tensor) -> torch.Tensor:
        padded_tensor = torch.nn.functional.pad(tensor_.unsqueeze(0),
        ...
        for x in range(0, dim_x, self.inner_patch_size):
            for y in range(0, dim_y, self.inner_patch_size):
                offset_x = 0  # If the patch is at the end and the padding doesn't provide enough pixels
                ...
                p = padded_tensor[..., x - offset_x: x + self.inner_patch_size + 2 * self.padding,
                    y - offset_y: y + 2 * self.padding + self.inner_patch_size]
                patches.append(p.unsqueeze(0))
        return torch.cat(patches, dim=0)

    def group_tensor(self, tensors: torch.Tensor, orig_size: Union[torch.Size, Tuple[int, int, int]]) -> torch.Tensor:
        dim_x = orig_size[1]
        dim_y = orig_size[2]
        ...
        for x in range(0, dim_x, self.inner_patch_size):
            for y in range(0, dim_y, self.inner_patch_size):
                ...
                if x + self.inner_patch_size > dim_x or y + self.inner_patch_size > dim_y:
                    ...
                    offset_y = max(0, y + self.inner_patch_size - dim_y)
                if self.padding > 0:
                    final[..., x: x + self.inner_patch_size, y: y + self.inner_patch_size] = tensors[i, ...,
