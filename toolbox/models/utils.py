...
def get_model(in_c: int,
    ...
    if n_layers == 1:
        model = nn.Sequential(
            nn.Conv2d(in_c, out_c, KERNEL_SIZE, STRIDE, PADDING, bias=True, padding_mode=PADDING_MODE),
            get_activation(last_activation)
        )
    else:
        layers = [
            nn.Conv2d(in_c, mid_c, KERNEL_SIZE, STRIDE, PADDING, bias=True, padding_mode=PADDING_MODE),
            get_activation(mid_activation)
        ]
        for i in range(1, n_layers - 1):
            layers.append(nn.Conv2d(mid_c, mid_c, KERNEL_SIZE, STRIDE, PADDING, bias=False, padding_mode=PADDING_MODE))
            if use_batchnorm:
                layers.append(nn.BatchNorm2d(mid_c))
            layers.append(get_activation(mid_activation))
        layers.append(nn.Conv2d(mid_c, out_c, KERNEL_SIZE, STRIDE, PADDING, bias=False, padding_mode=PADDING_MODE))
