...
def get_model(
    ...
    if n_layers == 1:
        model = nn.Sequential(
            nn.Conv2d(
                in_c, out_c, KERNEL_SIZE, STRIDE, PADDING, bias=True, padding_mode=PADDING_MODE
            ),
        ...
        layers = [
            nn.Conv2d(
                in_c, mid_c, KERNEL_SIZE, STRIDE, PADDING, bias=True, padding_mode=PADDING_MODE
            ),
        ...
        layers.append(
            nn.Conv2d(
                mid_c, out_c, KERNEL_SIZE, STRIDE, PADDING, bias=False, padding_mode=PADDING_MODE
            )
        )
